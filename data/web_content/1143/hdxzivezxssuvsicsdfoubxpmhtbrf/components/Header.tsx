        ...
        <div className="flex items-center">
          ...
          <div className="hidden md:flex items-center ml-8 space-x-2 border-l border-gray-700 pl-6">
            <button 
              ...
              className={`px-2 py-1 text-sm rounded-md transition-colors ${language === 'ca' ? 'text-background bg-accent' : 'text-secondary hover:text-primary'}`}
          ...
          <nav className="flex flex-col items-center px-6 pt-2 pb-4 space-y-1">
            {navLinks.map(([key, text]) => (
             ...
             <div className="flex items-center w-full justify-center mt-4 space-x-4 border-t border-gray-700 pt-4">
              <button 
                onClick={() => { setLanguage('ca'); setIsMenuOpen(false); }} 
                className={`px-4 py-2 text-sm rounded-md transition-colors ${language === 'ca' ? 'text-background bg-accent' : 'text-secondary hover:text-primary'}`}
              >
                CA
              </button>
              <button 
