from libs.utils.paths import get_cid_header_path


def generate_header_file(row):
    ...
    # Base buttons for desktop
    desktop_buttons = ""
    mobile_buttons = ""

    if min_lang != "none":
        desktop_buttons += f"""            <button 
              onClick={{() => setLanguage('{min_lang}')}} 
              className={{`px-2 py-1 text-sm rounded-md transition-colors ${{language === '{min_lang}' ? 'text-background bg-accent' : 'text-secondary hover:text-primary'}}`}}
              aria-pressed={{language === '{min_lang}'}}>
              {min_lang.upper()}
            </button>
"""
        mobile_buttons += f"""              <button 
                onClick={{() => {{ setLanguage('{min_lang}'); setIsMenuOpen(false); }}}} 
                className={{`px-4 py-2 text-sm rounded-md transition-colors ${{language === '{min_lang}' ? 'text-background bg-accent' : 'text-secondary hover:text-primary'}}`}}
              >
                {min_lang.upper()}
              </button>
"""

    # Always add Spanish and English buttons
    desktop_buttons += """            <button 
...
"""
    ...
    with open(get_cid_header_path(municipio_id, cid), "w", encoding="utf-8") as f:
        f.write(content)
